...
- 按「▶ 重做這些題目」可以整批重做一次 —— 重做時**不會給答案**，交卷後才批改
...
│   │                         results, ai, manage, practice, notifications
...
```bash
npm test                # 單元測試（54 項：批改、換算、驗證、匯入、自動組卷、SMTP、速率限制）
npm start               # 另開一個終端機
node test/e2e.js        # 端對端測試（321 項）：登入 → 作答 → 交卷 → 批改 → 成績單
                        # → 老師改分 → 檔案管理 → 成績批次操作 → 保留政策與清理
                        # → 成員管理 → 題庫組卷 → 人機驗證 → AI 背景工作
                        # → 螢光筆畫記 → 素材保全 → 題目編輯器 → 錯題複習
                        # → 自動組卷 → 通知 → Email 設定 → 穩定性防線 → 速率限制
```
