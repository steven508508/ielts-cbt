...
```bash
npm test                # 單元測試（145 項：批改、換算、驗證、匯入、自動組卷、SMTP、紀律分級、環境診斷、出題難度、檢討素材、Realtime 協定、題型可作答性、口說換手規則、考官設定、收音診斷、考官發話排隊）
npm start               # 另開一個終端機
node test/e2e.js        # 端對端測試（454 項）：登入 → 作答 → 交卷 → 批改 → 成績單，含十一種題型逐一走完全程
