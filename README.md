...
[![DOI](https://zenodo.org/badge/DOI/10.5281/zenodo.22181834.svg)](https://doi.org/10.5281/zenodo.22181834)

[繁體中文](README.md) · **[English](README.en.md)**

### ▶ [線上示範站 — 不用註冊，點進去就能考](https://steven508508.github.io/ielts-cbt/)

跑的是這個專案**真正的前端程式碼**，後端換成瀏覽器裡的模擬層。閱讀、聽力、
寫作都能實際作答，聽力有真的音檔，口說是一段錄好的示範對話（考官會出聲，
但不會用到你的麥克風）。聽力與閱讀的批改直接用 `server/lib/answers.js`
與 `server/lib/bands.js`，跟正式站同一份程式碼，所以分數換算完全一樣。
重新整理就回到全新狀態。
...
- 改題幹、選項、標準答案、解析、題號
- **閱讀原文與聽力音檔、逐字稿**就在每一節的最上面，不必再跳到別的畫面
- **寫作**可以改 Task 編號、最低字數、作答時間、圖表描述、圖片與範文
- **口說**可以改 Part 編號、主題、題目清單；Part 2 另有提示卡主題、
  要點清單、準備與說話秒數，以及收尾追問
- 新增／刪除題目與整個題組，也可以新增或刪除一整節。
  在口說或寫作題組按新增，會直接給你正確的 Part / Task 骨架
- 存檔前會先驗證：題號重複、答案不合法（例如 T/F/NG 填了 MAYBE）、
  沒填答案、`bodyHtml` 的 `[[題號]]` 對不上，
  **以及「這一題學生根本按不下去」**（單選／多選沒有選項、
  題目既沒有題幹也沒有空格），都會列出來讓你先修
- 插題刪題造成題號跳號時，按「**↻ 重新編號**」可以整科重排
  （已經考過的試卷不要動，成績是照題號對應的）

圖片請到「檔案」上傳後把網址貼過來；也可以用同一列的「素材」按鈕
一次把整份試卷缺的文章、音檔、圖片補齊。
...
前端的倒數只是**顯示**。真正的判定與收卷都在伺服器：

- **倒數用伺服器的時鐘。** 以前是拿 `endsAt`（伺服器算的）減 `Date.now()`
  （學生電腦的時鐘）—— 學生的電腦慢十分鐘就多考十分鐘，快十分鐘則一進去
  就被判定時間到。現在開始作答時量一次時鐘差，並每 20 秒跟伺服器對一次時。
- **分頁切到背景也收得掉。** 瀏覽器會把背景分頁的計時器節流到一分鐘一次，
  「時間到該收卷」那一刻很可能整個被跳過。回到前景會立刻補對時；
  就算學生直接關掉分頁、闔上筆電，伺服器每 30 秒的掃描一樣會把逾時的科目
  收掉，每一科都結束就整份自動交卷並開始批改。
- **最後一科結束就立刻收卷**，不用等下一輪掃描。掃描是給「前端沒跑到」的
  情況用的保底。
- **自動收卷會留紀錄**（成績頁的「考試紀律」看得到），老師分得出是學生自己
  交的、時間到系統收的、還是全部考完自動收的。
- 逾時之後 30 秒內仍接受作答（留給網路延遲），超過就一律退回。
- 另有一道 6 小時的硬性上限，防止 state 壞掉的紀錄永遠掛在「考試中」。
...
| 允許離開幾次 | 0 = 不設上限只記錄；設 3 就是「離開 3 次沒事，**第 4 次**才處置」 |
| 超過上限時 | **只警告**（建議）或 **自動結束該科**（由伺服器執行，不是靠前端） |
...
切一次分頁，瀏覽器其實會連續丟出好幾個訊號（先 `blur`、再 `visibilitychange`）。
系統把「離開」當成一個**狀態**而不是一連串事件：出去記一次，回來才重新開始算，
所以學生切走一次就是一次。

「幾次」這個數字全站只有一個定義（`conduct.LEAVE_WHERE`）：
**離開畫面或離開全螢幕，而且沒有被降成「紀錄」**。回報事件當下、
重新整理考卷、成績頁三個地方都用同一份，不會出現學生一重新整理次數就跳上去的情形。

#### 上限是伺服器在管

超過上限的處置由**伺服器**判定並執行：直接把那一科標成已交卷、寫一筆
`auto_submit` 紀錄，之後不論是繼續存答案還是重新整理再進來，都會被擋下。
前端那段提示只是把結果講給學生聽而已 —— 把 JavaScript 停掉並不能繞過。
...
- **選項清單也會列出來**，並標出「你選的」與「正解」。配合題、選字填空的選項是
  整個題組共用的，所以畫在題組上方一次，不會在十三題底下重複十三遍
- 答案是 `ii` 這種字母時，會把對應的選項文字一起寫出來 ——
  只看到「你選了 i、正解是 ii」對檢討沒有幫助
...
- **斷線會自動接回來**：學生那條連線斷了會自動重連（最多 5 次，間隔逐次拉長），
  考官那一端掉線伺服器會自己重接（最多 3 次）並把先前的對話補給模型。
  接回來之後從原本的階段繼續，逐字稿與已答的題目都不會不見。
...
同一套設定不可能同時適合初級班和衝刺班，所以整組都開放調整。
管理員設**全站預設**，老師指派考試時可以針對特定班級**覆寫**，
沒改的項目就沿用上一層：
...
| 分組 | 可以調什麼 |
|---|---|
| 收音與語音偵測 | 語音偵測門檻、要不要開瀏覽器降噪 |
| 人設與口音 | 考官名字、聲音、英式／美式／澳式口音、語速、風格（親切／中性／嚴肅）、額外指示 |
| 換手靈敏度 | 停頓多久算講完、可不可以打斷考官 |
| 難度與追問 | 追問強度、評分寬嚴、Part 1／3 長度、Part 2 準備與長回答秒數 |
| 學生看得到什麼 | 即時分數、逐字稿、階段提示、提示卡、音量指示 |

#### 考官講到一半就不出聲了

Realtime 端點一次只能有一個進行中的回應。系統要求考官講話時，如果剛好撞上
端點自己發起的回應，就會被回一句 `Conversation already has an active response`。
撞在**階段轉換**那一次的話，Part 1 的第一題永遠不會被問出來 —— 學生講完
姓名與出生地、考官回一句之後從此沉默。

現在改成排隊：要考官講話就記下來，等前一個回應真的結束再送。並且加了一層
會自救的看門狗：

| 情況 | 系統怎麼做 |
|---|---|
| 送出去 5 秒還沒開始講 | 自動重下一次（最多 3 次） |
| 第 2 次還救不回來 | 先告訴學生「正在重新叫醒考官」，同時繼續重試 |
| 重試用盡 | 請學生再說一次或按「叫考官接話」 |
| 標記成「正在講話」卻 15 秒沒半點聲音 | 判定狀態壞了，清掉重來 |

另外，考官的指示現在明確禁止它自己停下來：**每一次發言都必須以問題結尾**，
題庫裡的題目問完之後要繼續用同主題的自然問題把 Part 1／3 撐滿 4–5 分鐘，
不准宣布這一部分結束、不准等學生先開口 —— 什麼時候換階段由系統決定。
這正是官方考官的做法。

#### 考官聽不到學生時

這是最常見也最難查的一種：學生明明在講話，考官就是沒反應，逐字稿一片空白，
而畫面上**沒有任何線索**。系統現在會自己分辨並當場講出來：

| 情況 | 系統怎麼判斷 | 學生看到什麼 |
|---|---|---|
| 麥克風沒在收 | 送出的音量持續接近 0 超過 20 秒 | 「系統幾乎收不到任何聲音」+ 檢查步驟 |
| 收得到但不被當成說話 | 音量正常累計 10 秒，卻從沒被判定為說話 | 「考官那一端一直沒有判定成有人在說話」+ 處理方式 |
| 學生只是在思考 | 音量低但講話時正常 | 不打擾 |

前端每兩秒回報一次自己送出去的音量，伺服器靠這個才分得出「沒講話」和
「在講話但沒被聽見」—— 這兩者以前長得一模一樣。

根治要調「收音與語音偵測」那一組：

- **語音偵測門檻**（預設 0.4）。學生講話小聲就調低到 0.3，環境吵、常常誤判就調高。
- **瀏覽器降噪預設關閉**。Chrome 的降噪對穩定的人聲壓得很兇 —— 實測同一段訊號，
  開降噪的音量只有關掉的四分之一，壓完之後常常過不了偵測門檻。
  考場本來就安靜，不太需要降噪；吵的環境再開回來。

幾個實務上的取捨：

- **最常需要調的是「停頓多久算講完」**。預設 1.1 秒是一般程度的值；
  程度較低的班級講話停頓多，1.5–2 秒比較不會被搶話。
- **即時分數預設不給學生看**。真實 IELTS 不會邊考邊報分數，而且看到分數
  往下掉對正在講話的學生只有干擾。關掉不影響計分 —— 分數照常算、照常存，
  老師那邊看得到。
- **「額外指示」用英文寫效果最好**，會原樣加進考官的提示詞
  （例如 `Avoid topics about politics or religion.`）。評分規則不要寫在這裡，
  那有專門的「評分寬嚴」。
- **Part 2 的準備與長回答，考官一律不會自動接話** —— 這是官方規則，不開放調整。
- 「🔊 試聽開場白」可以在推給學生之前先聽聽看這個聲音與名字的效果。
- 老師改不了系統層的設定，只能在自己的指派裡覆寫。

指派層**只存跟系統預設不一樣的欄位**。所以管理員之後調整全站預設時，
既有的指派會跟著動 —— 除了老師刻意改過的那幾項。

#### 換手規則是分階段的

「什麼時候算學生講完了、考官該接話」不是整場一套。官方 Part 2 的考官
**不能打斷**考生，而考生講一兩分鐘本來就會停下來想 ——
所以停頓多久算結束、要不要自動接話，每個階段各自設定：

| 階段 | 停頓多久算講完 | 停頓後自動接話 | 學生可以打斷考官 |
|---|---|---|---|
| 介紹 / Part 1 / Part 3 / Part 2 收尾 | 1.1 秒 | ✓ | ✓ |
| Part 2 讀題 | 1.1 秒 | ✓ | ✗（讀題不該被咳嗽打斷）|
| Part 2 準備的一分鐘 | 2 秒 | ✗（考官全程閉嘴）| — |
| Part 2 長回答 | 2 秒 | ✗（官方規則不能打斷）| — |

「不自動接話」只是不讓考官插嘴，**語音辨識照常跑**，逐字稿一句都不會少。
Part 2 時間到之後由伺服器明確叫考官收尾。

打斷考官的判斷放在**伺服器**：只有它知道考官到底在不在講話。
前端無條件送取消的話，沒戴耳機時考官自己的聲音會被麥克風收回去，
考官等於把自己講到一半的話砍掉。
...
> 收件者名單裡有已停用或已刪除的帳號時會自動略過，並回報略過了幾個 ——
> 多筆 INSERT 是一句 SQL，撞到外鍵會整批失敗，一個壞帳號不能害同批的其他人收不到。

**紀錄不會無限長。** 每次送通知時就順手修剪：

| | 上限 |
|---|---|
| 同一個帳號 | 最近 200 則（含未讀 —— 累積兩百則未讀的人顯然沒在看） |
| 已讀的硬上限 | 180 天 |

這是寫在送通知的流程裡的，不依賴「資料管理」那邊的自動清理（那個預設是關的）。
實測一張累積到三萬筆的表，下一次送通知之後降到 200 筆，而送通知本身只花 8 ms
（修剪在背景做，不擋回應）。

> 參考數量級：一則通知約佔 **278 bytes**（含索引）。500 人的學校一年大約
> 兩萬到五萬則，也就是 **5–15 MB** —— 比口說錄音的 50–150 GB 小三個數量級，
> 真正要盯的是錄音，不是通知。
...
| AI 呼叫紀錄保留（天） | 除錯用的日誌 |
| 監考事件紀錄保留（天） | 離開畫面／全螢幕的紀錄；成績與作答不受影響 |
| 維護紀錄保留（天） | 誰在什麼時候刪了什麼 |
...
- **磁碟會被口說錄音吃掉**：每一場口說會存一份完整錄音加十幾段分題錄音，
  一場約 8–25 MB。一年六千場就是 50–150 GB。請到「資料管理 → 自動清理」
  把「口說錄音保留（月）」設好並**開啟每天自動清理** —— 預設是關的，
  不開的話錄音會永遠留著。
...
```bash
npm test                # 單元測試（162 項：批改、換算、驗證、匯入、自動組卷、SMTP、紀律分級、環境診斷、出題難度、檢討素材、Realtime 協定、題型可作答性、口說換手規則、考官設定、收音診斷、考官發話排隊、考試時限）
npm start               # 另開一個終端機
node test/e2e.js        # 端對端測試（464 項）：登入 → 作答 → 交卷 → 批改 → 成績單，含十一種題型逐一走完全程
                        # → 老師改分 → 檔案管理 → 成績批次操作 → 保留政策與清理
                        # → 成員管理 → 題庫組卷 → 人機驗證 → AI 背景工作
                        # → 螢光筆畫記 → 素材保全 → 題目編輯器 → 錯題複習
                        # → 考試授權 → 自動組卷 → 通知 → Email 設定 → 出題難度 → 檢討素材
                        # → 考前環境診斷
                        # → 紀律事件分級 → 穩定性防線 → 速率限制
```

口說的即時對話另外有一支瀏覽器實測。**只有真的跑一次瀏覽器才抓得到**的問題
（例如音訊擷取的節點沒接好、整條線根本不會被拉）用 API 測試全部都是綠的：
...
Playwright 刻意不列進相依套件 —— 它會讓 Docker 映像檔大好幾百 MB，
而這支測試是開發時才需要跑的。
